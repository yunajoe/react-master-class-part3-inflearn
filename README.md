...
1. 🎯 핵심 아키텍처 원칙: Observer vs Commander
   | 비교 항목 | `useQuery` (관찰자 👁️) | `useMutation` (행동 대장 ⚡) |
   | :--- | :--- | :--- |
   | **실행 방식** | **선언적 / 자동** (컴포넌트 마운트·키 변경 시 자동 실행) | **명령형 / 수동** (버튼 클릭·폼 제출 등 직접 호출 시 실행) |
   | **현실 비유** | 메뉴판을 보며 신메뉴가 나왔는지 확인하는 과정 | 벨을 눌러 점원에게 "이 메뉴로 주문할게요!"라고 외치는 액션 |
   | **주요 역할** | **Read** (서버 데이터 조회 및 상태 관찰) | **Create, Update, Delete** (서버 데이터 생성·수정·삭제) |

---

2. Mutation 최종 테스트 케이스 및 체크포인트 요약

| 번호  | 테스트 항목                      | 검증 방법 및 동작 확인                                                          | 핵심 가치                                   |
| :---- | :------------------------------- | :------------------------------------------------------------------------------ | :------------------------------------------ |
| **1** | **방아쇠(Trigger) 동작**         | 페이지 로드 시 네트워크 요청 **0건** 확인 ➔ 버튼 클릭 시에만 요청 발생          | `useMutation` 수동적(명령형) 실행 구조 검증 |
| **2** | **중복 요청 방지 (`isPending`)** | 통신 지연(2초) 동안 버튼 연타 ➔ 버튼 비활성화(`disabled`) 유지                  | UX 보호 및 서버 중복 요청 방지              |
| **3** | **피드백 루프 (`onError`)**      | 10% 확률 에러 발생 시 ➔ 얼럿/로그에 **요청 시 전달한 데이터(`variables`)** 표출 | 정밀한 에러 추적 및 사용자 피드백           |
| **4** | **프로세스 종료 (`onSettled`)**  | 성공/실패 여부와 관계없이 ➔ `🏁 모든 통신 프로세스 종료` 로그 출력              | 안전한 리소스 정리 및 로딩 상태 해제        |
...
[버튼 클릭 (mutate)]
       │
       ▼
[isPending: true] ──▶ 버튼 비활성화 (중복 클릭 방지)
       │
       ▼
[서버 요청 처리 (2초)]
       │
 ...
 ▼                              ▼
[성공 (90%)]                  [실패 (10%)]
 ├─ onSuccess 실행             ├─ onError 실행
 │  (서버 응답 data 활용)       │  (실패 원인 error & 요청 variables 활용)
 └──────────────┬───────────────┘
                │
                ▼
...
| :---: | :--------------- | :---------------------------------------------------- | :--------------------------------------------------- |
| **1** | **`TData`**      | 성공 시 반환되는 데이터의 타입                        | `mutationFn` 리턴값, `onSuccess(data)`의 `data`      |
