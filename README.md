...
- 관리 영역(Domain)을 조각내어 유지보수의 독립성을 확보하는 아키텍처
...
- 슬라이스를 만들 때 가장 중요한 문법은 StateCreator
...
```markdown
- 첫번째 인자 (DepartmentStore): 전문가가 나중에 합쳐질 건물 전체의 구조를 인지해야 함을 뜻합니다. 그래야 1층 전문가가 get()을 통해 다른 층 상황을 참조할 수 있습니다.
