...
### 77. 의존적 쿼리(Dependent Queries)

1. 도입 배경 및 개념

- 문제점: 이전 요청의 결과값(예: userId)이 있어야만 다음 요청을 보낼 수 있는 순차적 호출 상황에서, 동시에 호출할 경우 Parameter 부재로 인한 API 에러 발생.

- 해결책 (enabled): 조건문 기반으로 쿼리의 실행 여부를 선언하여 비동기 호출의 순서를 제어하는 게이트키퍼 역할 수행.

2. 핵심 용어

- Dependent Query: 특정 쿼리가 실행되기 위해 이전 쿼리의 결과값이 반드시 필요한 상태.

- enabled 옵션: true일 때만 queryFn을 실행하도록 쿼리 엔진의 출항을 통제하는 설정.

- Waterfall (네트워크 워터폴): 의도적으로 요청을 계단식(순차적)으로 발생시켜 데이터 순서를 보장하는 현상.

- Non-null assertion (! operator): 타입스크립트에게 "이 값은 absolute undefined/null이 아니다"라고 명시하는 기법.

3. 엔진 내부 동작 흐름

- 관찰 및 대기: 컴포넌트 마운트 시 enabled가 false면 요청을 보내지 않고 Pending 상태 유지 (네트워크 요청 발생 X).

- 트리거 발생: 이전 쿼리가 성공하여 userId 등의 의존 데이터가 확보되는 순간 enabled 조건이 true로 변경.

- 빗장 해제: 엔진이 조건 변화를 감지하고 즉시 두 번째 queryFn 호출.

- 타입 확정: 조건 완결 후 실행되므로, userId! 형태로 안전하게 타입 확정 후 로직 수행.

| 구분            | useEffect 수동 호출 (비권장)         | enabled 옵션 활용 (권장)                  |
| :-------------- | :----------------------------------- | :---------------------------------------- |
| **코드 스타일** | 명령형 (타이밍과 호출을 직접 제어)   | 선언적 (실행될 조건만 명시)               |
| **코드 복잡도** | useEffect 및 추가 상태 관리로 복잡   | 쿼리 옵션 하나로 간결하게 해결            |
| **예외 처리**   | 레이스 컨디션(Race Condition)에 취약 | 쿼리 엔진이 상태 동기화 및 캐싱 자동 관리 |
| **타입 안전성** | null/undefined 체크 누락 가능성 높음 | 조건 가드 기반으로 안전한 단정(`!`) 가능  |

### 78. 인터넷이 끊겼을 때 앱이 멈추는 현상

1. 도입 배경 및 개념

- 문제점: 오프라인 상황이나 새로고침 시 메모리에만 의존하는 캐시 데이터가 즉시 증발하여 유저 경험(UX)이 심각하게 훼손됨.

- 해결책 (Persistence): 캐시 데이터를 브라우저의 외부 저장소(Local Storage 등)에 박제하고, 앱 재실행 시 메모리로 복구(Hydration)하여 끊김 없는 오프라인 경험을 제공하는 전략.

2. 핵심 용어

- Persister (퍼시스터): 탄스택 쿼리 엔진과 외부 저장소를 연결해 캐시 데이터를 저장소에 실시간 동기화하는 전달자.

- Hydration (하이드레이션): 앱 실행 시 외부 저장소의 박제 데이터를 읽어와 쿼리 엔진의 메모리를 즉시 채우는 수분 공급 과정.

- gcTime (Garbage Collection Time): 캐시 데이터가 메모리에서 유지되는 시간으로, 오프라인 지원을 위해 길게(예: 24시간) 설정.

- Buster (버스터): 데이터 구조 변경 시 이전 버전의 낡은 캐시를 강제로 삭제하고 새로운 구조를 받아오게 하는 버전 식별자.

3. 알맹이(Data) vs 껍데기(Asset) 구별

- 쿼리 지속성 한계: 서버 데이터(JSON 알맹이)를 저장소에 박제하여 오프라인 데이터 조회를 가능하게 만듦
